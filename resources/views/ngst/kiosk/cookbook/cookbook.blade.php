        ...
        @if(app()->getLocale() === 'fr-ca')
            @foreach($categories as $key => $category)
                <div class="col-sm-4 text-center" style="margin-bottom:3%">
                    <a href="/{{app()->getLocale()}}/kiosk/cookbook/{{$category->id}}"><input class="btn btn-primary form-control" style="font-size:18px;padding:15%" value="{{$category->name_fr}}" readonly></a>
                </div>
            @endforeach
        @else
            @foreach($categories as $key => $category)
                <div class="col-sm-4 text-center" style="margin-bottom:3%">
                    <a href="/{{app()->getLocale()}}/kiosk/cookbook/{{$category->id}}"><input class="btn btn-primary form-control" style="font-size:18px;padding:15%" value="{{$category->name_en}}" readonly></a>
                </div>
            @endforeach
        @endif
    </div>
